...
function highlightOwnName(authors: string) {
  const parts = authors.split(aboutMe.name);
  return parts.map((part, i) => (
    <span key={i}>
      {i > 0 && (
        <span className="font-semibold text-zinc-900">{aboutMe.name}</span>
      )}
      {part}
    </span>
  ));
      ...
      {publication.projectUrl && (
        ...
        >
          <ArrowUpRight
            ...
            className="group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-300"
          />
          <span className="tracking-wider uppercase">Webpage</span>
        </a>
      )}
      {publication.bibtex && (
        <a
  ...
  return (
    <div className="flex flex-col sm:flex-row gap-6">
      {publication.imageUrl && (
        <div className="w-full sm:w-1/4 min-w-[160px] relative flex flex-col gap-3">
          <Image
